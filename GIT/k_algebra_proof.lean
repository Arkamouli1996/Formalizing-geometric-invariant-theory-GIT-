...
/-!
Step 4 is where the handwritten proof does the degree induction. The initial attempt below
tried to *manufacture* a representation

`y = ∑ i, a i * t i`

with each `t i ∈ T` homogeneous of positive degree and each `a i` itself homogeneous of strictly
smaller degree.

In Lean, that forces you to prove a global equality in `R` by comparing *all* graded components
(`ext j` after applying `DirectSum.decompose`), which is the most bookkeeping-heavy part of the
formalization.

New flow (shorter in practice):

- keep an arbitrary expression for `y` coming from `y ∈ Ideal.span (T : Set R)`:

    `y = ∑ t ∈ l.support, l t * t`

  where the coefficients `l t : R` are not homogeneous.
- use strong induction on `d` to show **each homogeneous component** of each `l t` of degree
  `< d` lies in the adjoin (by IH), and then conclude the full coefficient `l t` lies in the adjoin
  because it is a *finite sum* of its homogeneous components.
- close under multiplication by `t ∈ T` and finite sums.

This avoids constructing new index types and avoids proving an auxiliary “homogeneous-coefficients
sum equals `y`” lemma.
-/

/-!
Bookkeeping lemma still needed for Step 4 (TODO):

If `t ∈ 𝒜 n` is homogeneous, then the degree-`d` component of `(r * t)` depends only on the
degree-`(d - n)` component of `r`.  This is the graded multiplication/projection lemma used to
justify that only degrees `< d` appear on the coefficient side when `n > 0`.

Once this lemma is in place, the induction in
`homogeneous_mem_adjoin_of_irrelevant_eq_span` becomes a fairly direct closure argument.
-/
lemma decompose_mul_homogeneous_component
    {d n : ℕ} {r t : R}
    (ht : t ∈ 𝒜 n) :
    ((DirectSum.decompose 𝒜 (r * t)) d : R) =
      ((DirectSum.decompose 𝒜 (((DirectSum.decompose 𝒜 r) (d - n) : R) * t)) d : R) := by
  classical
  by_cases hle : n ≤ d
  · -- both sides evaluate to the same product `(decompose r (d-n)) * t` in degree `d`
    have hL :
        ((DirectSum.decompose 𝒜 (r * t)) d : R) =
          ((DirectSum.decompose 𝒜 r) (d - n) : R) * t := by
      -- projection lemma for `a * b` with `b` homogeneous on the right
      simpa using
        (DirectSum.coe_decompose_mul_of_right_mem_of_le (𝒜 := 𝒜) (a := r) (b := t) (n := d) (i := n) ht hle)
    have hR :
        ((DirectSum.decompose 𝒜 (((DirectSum.decompose 𝒜 r) (d - n) : R) * t)) d : R) =
          ((DirectSum.decompose 𝒜 r) (d - n) : R) * t := by
      -- apply the same projection lemma again, now with `a := (decompose r) (d - n)`
      -- and then use that this `a` is homogeneous of degree `d - n`.
      have hproj :
          ((DirectSum.decompose 𝒜 (((DirectSum.decompose 𝒜 r) (d - n) : R) * t)) d : R) =
            ((DirectSum.decompose 𝒜 ((DirectSum.decompose 𝒜 r) (d - n) : R)) (d - n) : R) * t := by
        simpa using
          (DirectSum.coe_decompose_mul_of_right_mem_of_le (𝒜 := 𝒜)
            (a := ((DirectSum.decompose 𝒜 r) (d - n) : R)) (b := t) (n := d) (i := n) ht hle)
      -- the `d - n`-component of a homogeneous element of degree `d - n` is itself
      have ha_mem : ((DirectSum.decompose 𝒜 r) (d - n) : R) ∈ 𝒜 (d - n) :=
        ((DirectSum.decompose 𝒜 r) (d - n)).2
      have hcomp :
          ((DirectSum.decompose 𝒜 ((DirectSum.decompose 𝒜 r) (d - n) : R)) (d - n) : R) =
            ((DirectSum.decompose 𝒜 r) (d - n) : R) := by
        simpa using (DirectSum.decompose_of_mem_same 𝒜 (x := ((DirectSum.decompose 𝒜 r) (d - n) : R))
          (i := d - n) ha_mem)
      simpa [hcomp] using hproj
    exact hL.trans hR.symm
  · -- if `n > d`, both sides have degree-`d` component equal to `0`
    have hL : ((DirectSum.decompose 𝒜 (r * t)) d : R) = 0 := by
      simpa using
        (DirectSum.coe_decompose_mul_of_right_mem_of_not_le (𝒜 := 𝒜) (a := r) (b := t) (n := d) (i := n) ht hle)
    have hR : ((DirectSum.decompose 𝒜 (((DirectSum.decompose 𝒜 r) (d - n) : R) * t)) d : R) = 0 := by
      simpa using
...
/- Step 5: package the induction into an `Algebra.FiniteType (𝒜 0) R` statement.
